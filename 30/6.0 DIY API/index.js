...
function getJokeById(id) {
  ...
  // another way
  //result = jokes.find((joke) => joke.id === id);
  return result;
}
...
function getJokesByType(type) {
  return jokes.filter((joke) => joke.jokeType.toLowerCase() === type.toLowerCase());
}
...
app.get("/jokes/:id", (req, res) => {
    var joke = getJokeById(parseInt(req.params.id));
    res.json(joke);
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req, res) => {
  var foundJokes = getJokesByType(req.query.type);
  res.json(foundJokes);
});
